variable "schematics_policies" {
  ...
  validation {
    condition = alltrue([
      ...
      try(p.location, null) == null || contains(["us-south", "us-east", "eu-gb", "eu-de"], try(p.location, null)))
    ])
    error_message = "Invalid Schematics policy location. Allowed values are: us-south, us-east, eu-gb, eu-de."
  }
        ...
        for param in try(p.parameter, []) : [
          for ap in try(param.agent_assignment_policy_parameter, []) : [
            for scope in try(ap.selector_scope, []) : alltrue([
              ...
              contains(["us-south", "us-east", "eu-gb", "eu-de"], loc)
            ])
          ]
        ]
      ]
    ]))
    error_message = "Invalid selector_scope.locations in agent_assignment_policy_parameter. Allowed values are: us-south, us-east, eu-gb, eu-de."
  }
  ...
  validation {
    ...
    error_message = "Invalid target.selector_scope.locations. Allowed values are: us-south, us-east, eu-gb, eu-de."
  }
}
